...
CREATE TABLE leave_record (
    ...
    status          VARCHAR(10)  NOT NULL DEFAULT '待审批' COMMENT '审批状态(待审批/已批准/已驳回)',
    ...
    CONSTRAINT chk_leave_status  CHECK (status IN ('待审批', '已批准', '已驳回')),
    CONSTRAINT chk_leave_date   CHECK (end_date >= start_date)
) ENGINE=InnoDB COMMENT='请假记录表';

CREATE INDEX idx_leave_student_id ON leave_record(student_id);
CREATE INDEX idx_leave_status     ON leave_record(status);
CREATE INDEX idx_leave_apply_time ON leave_record(apply_time);
CREATE INDEX idx_leave_teacher_id ON leave_record(teacher_id);
CREATE INDEX idx_leave_date_range ON leave_record(start_date, end_date);
...
SELECT
    ...
    SUM(CASE WHEN lr.status = '已批准' THEN 1 ELSE 0 END) AS approved_count,
    SUM(CASE WHEN lr.status = '已驳回' THEN 1 ELSE 0 END) AS rejected_count,
    SUM(CASE WHEN lr.status = '待审批' THEN 1 ELSE 0 END) AS pending_count
FROM class c
LEFT JOIN student s   ON c.class_id = s.class_id
LEFT JOIN leave_record lr ON s.student_id = lr.student_id
